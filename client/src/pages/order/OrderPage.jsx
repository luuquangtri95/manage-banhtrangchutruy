  ...
  delivery_date: {
    ...
    validate: (value) => {
      ...
      if (inputDate < currentDate)
        return "order_page.validate.date_must_be_today_or_later";
...
function OrderPage() {
  ...
  const handleChangeProduct = (itemPicked) => {
    ...
    const newProducts = itemPicked.map((item) => ({
      ...item,
      quantity:
        originProduct.find((p) => p.value === item.value)?.quantity || 1,
    }));
  ...
  const handleRemoveProductPicked = (itemPicked) => {
    ...
    setOriginProduct(
      _originProduct.filter((_item) => _item.value !== itemPicked.value)
    );
  };
